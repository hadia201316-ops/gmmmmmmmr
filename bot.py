...
BOT_TOKEN = "8912123959:AAFWZ8fQrjD_jCMw8Ye9wKfIAY_HDPxYZjw"
OWNER_ID = 6965052196
...
class UserbotSession:
    def __init__(self, client, user_id, session_key):
        ...
        # ============================================================
        # تهيئة نظام م9 (التتبع والتحويل التلقائي)
        # ============================================================
        self.tracking_system = م9(self.client, self.user_id)
...
"""
    ...
    async def stop(self):
        ...
        # إيقاف نظام م9
        if hasattr(self, 'tracking_system'):
            await self.tracking_system.stop()
...
# ====================================================================
# كلاس م9 - نظام التتبع والتحويل التلقائي من الرسائل المحفوظة
# ====================================================================
class م9:
    """
    كلاس م9: نظام التتبع والتحويل التلقائي من الرسائل المحفوظة (Saved Messages).
    """

    def __init__(self, client, user_id):
        """
        تهيئة كلاس م9.
        :param client: كائن العميل (TelegramClient) الخاص بالمستخدم.
        :param user_id: معرف المستخدم (صاحب الجلسة).
        """
        self.client = client
        self.user_id = user_id
        # قاموس لتخزين عمليات التتبع النشطة
        # المفتاح: (chat_id, target_user_id) -> القيمة: { "task": asyncio.Task, "speed": float, "username": str }
        self.tracking_tasks = {}
        self.tracking_lock = asyncio.Lock()
        # متغيرات للتحكم في التشغيل
        self.running = True
        self.my_id = None

        # بدء تشغيل معالج الأوامر الخاص بالكلاس
        asyncio.create_task(self.setup_handlers())

        # جلب معرف المستخدم الحالي
        asyncio.create_task(self._init_user_id())

        print(f"[م9] تم تهيئة نظام التتبع للمستخدم {self.user_id}")
    ...
    async def _init_user_id(self):
        ...
        try:
            ...
            print(f"[م9] خطأ في جلب معرف المستخدم: {e}")

    # ========================================================================
    # دوال مساعدة
    # ========================================================================

    async def _get_user_entity(self, user_input):
        """
        الحصول على كيان المستخدم من معرف أو يوزر.
        :param user_input: معرف رقمي أو يوزر المستخدم (مثل @username).
        :return: كيان المستخدم أو None.
        """
        try:
            if isinstance(user_input, int):
                return await self.client.get_entity(user_input)
    ...
    async def _get_user_id_from_reply_or_link(self, event):
        """
        استخراج معرف المستخدم من الرد أو من رابط الرسالة.
        :param event: حدث الرسالة.
        :return: (user_id, username) أو (None, None).
        """
        # محاولة استخراج من الرد
        if event.is_reply:
        ...
        # محاولة استخراج من رابط الرسالة في النص
        text = event.raw_text
        # البحث عن رابط t.me في النص
        match = re.search(r't\.me/([^/]+)/(\d+)', text)
        if match:
            chat_part = match.group(1)
            msg_id = int(match.group(2))
            try:
                # محاولة جلب الكيان من الجزء الأول من الرابط
                if chat_part.isdigit():
    ...
    async def _get_user_id_from_event(self, event):
        """
        استخراج معرف المستخدم من الرد أو من اليوزر في النص.
        :param event: حدث الرسالة.
        :return: (user_id, username) أو (None, None).
        """
        # محاولة استخراج من الرد
        ...
        # محاولة استخراج من رابط الرسالة
        match = re.search(r't\.me/([^/]+)/(\d+)', text)
        if match:
            chat_part = match.group(1)
            msg_id = int(match.group(2))
    ...
    async def _forward_last_two_saved_messages(self, chat_id, target_user_id, speed):
        """
        تحويل آخر رسالتين من الرسائل المحفوظة من مستخدم معين إلى محادثة معينة.
        :param chat_id: معرف المحادثة الهدف.
        :param target_user_id: معرف المستخدم المستهدف.
        :param speed: سرعة الانتظار بين الرسالتين.
        :return: (True, "تم التحويل") أو (False, "خطأ").
        """
        try:
            # جلب آخر رسالتين من المحفوظات للمستخدم المستهدف
            messages = []
            async for msg in self.client.iter_messages("me", from_user=target_user_id, limit=2):
                messages.append(msg)

            if len(messages) < 2:
                # إرسال رسالة توضيح بعدم وجود رسالتين
                await self.client.send_message(
                    chat_id,
                    f"⚠️ لا توجد رسالتان كافيتان في الرسائل المحفوظة من هذا المستخدم."
                )
                return False, "لا توجد رسالتان كافيتان"

            # عكس القائمة لجعل الأقدم أولاً (للحفاظ على الترتيب)
            messages.reverse()

            # تحويل الرسالتين
            for i, msg in enumerate(messages):
                try:
                    # تحويل الرسالة كما هي مع الحفاظ على التعليق
                    await msg.forward_to(chat_id)
                    # الانتظار حسب السرعة المحددة (بين الرسالتين)
                    if i < len(messages) - 1:
                        await asyncio.sleep(speed)
                except Exception as e:
                    # إذا فشلت إحدى الرسالتين، نستمر في تحويل الأخرى
                    print(f"[م9] فشل تحويل رسالة: {e}")
                    continue

            return True, "تم التحويل بنجاح"
        except Exception as e:
            print(f"[م9] خطأ في تحويل الرسائل: {e}")
            return False, str(e)
    ...
    async def start_tracking(self, chat_id, target_user_id, username="", speed=0):
        """
        بدء تتبع مستخدم في محادثة معينة.
        :param chat_id: معرف المحادثة.
        :param target_user_id: معرف المستخدم المراد تتبعه.
        :param username: اسم المستخدم (اختياري).
        :param speed: سرعة التحويل (0, 1, 2, 3).
        :return: (True, "تم البدء") أو (False, "خطأ").
        """
        async with self.tracking_lock:
            # التحقق من وجود تتبع نشط بالفعل
            key = (chat_id, target_user_id)
            if key in self.tracking_tasks:
                return False, "تتبع نشط بالفعل"

            # إنشاء مهمة التتبع
            task = asyncio.create_task(self._track_user(chat_id, target_user_id, username, speed))

            # حفظ المهمة والسرعة
            self.tracking_tasks[key] = {
                "task": task,
                "speed": speed,
                "username": username,
                "chat_id": chat_id,
            ...
            return True, "تم بدء التتبع"

    async def stop_tracking(self, chat_id, target_user_id):
        """
        إيقاف تتبع مستخدم في محادثة معينة.
        :param chat_id: معرف المحادثة.
        :param target_user_id: معرف المستخدم المراد إيقاف تتبعه.
        :return: (True, "تم الإيقاف") أو (False, "لا يوجد تتبع نشط").
        """
        async with self.tracking_lock:
            key = (chat_id, target_user_id)
            if key not in self.tracking_tasks:
                return False, "لا يوجد تتبع نشط لهذا المستخدم"

            # إلغاء المهمة
            task_data = self.tracking_tasks[key]
            ...
            # حذف البيانات
            del self.tracking_tasks[key]
            return True, "تم إيقاف التتبع"

    async def stop_all_tracking(self, chat_id=None):
        """
        إيقاف جميع عمليات التتبع أو في محادثة معينة.
        :param chat_id: معرف المحادثة (اختياري).
        """
        async with self.tracking_lock:
            keys_to_remove = []
            for key, data in self.tracking_tasks.items():
                if chat_id is None or data["chat_id"] == chat_id:
                    # إلغاء المهمة
            ...
            return True, f"تم إيقاف {len(keys_to_remove)} عملية تتبع"

    async def set_speed(self, chat_id, target_user_id, speed):
        """
        تغيير سرعة التحويل لتتبع معين.
        :param chat_id: معرف المحادثة.
        :param target_user_id: معرف المستخدم.
        :param speed: السرعة الجديدة.
        :return: (True, "تم التحديث") أو (False, "لا يوجد تتبع").
        """
        async with self.tracking_lock:
            key = (chat_id, target_user_id)
            if key not in self.tracking_tasks:
            ...
            # تحديث السرعة
            self.tracking_tasks[key]["speed"] = speed
            return True, f"تم تحديث السرعة إلى {speed}"

    # ========================================================================
    # دالة التتبع الأساسية (تعمل في الخلفية)
    # ========================================================================

    async def _track_user(self, chat_id, target_user_id, username, speed):
        """
        دالة التتبع الأساسية التي تعمل في الخلفية.
        تراقب الرسائل الجديدة من المستخدم المستهدف وتقوم بتحويل الرسائل.
        """
        # متغير لتتبع آخر رسالة تمت معالجتها
        last_processed_msg_id = None

        try:
            # الحصول على اسم المستخدم للتنبيهات
            display_name = f"@{username}" if username else f"المستخدم {target_user_id}"

            # إرسال رسالة تأكيد بدء التتبع
            await self.client.send_message(
                chat_id,
                f"✅ بدأ تتبع {display_name} بنجاح!"
            )

            while self.running:
                # التحقق من وجود التتبع في القاموس
                async with self.tracking_lock:
                    key = (chat_id, target_user_id)
                    if key not in self.tracking_tasks:
                        # تم إيقاف التتبع
                        break
                    # جلب السرعة الحالية
                    current_speed = self.tracking_tasks[key]["speed"]

                try:
                    # جلب آخر رسالة من المستخدم في المحادثة
                    last_msg = None
                    async for msg in self.client.iter_messages(chat_id, from_user=target_user_id, limit=1):
                        if msg and msg.id != last_processed_msg_id:
                            last_msg = msg
                            break
                    ...
                    if last_msg:
                        ...
                        # تحويل آخر رسالتين من المحفوظات
                        success, message = await self._forward_last_two_saved_messages(
                            chat_id, target_user_id, current_speed
                        )

                        if not success:
                            # إرسال رسالة الخطأ فقط إذا لم تكن بسبب عدم وجود رسائل كافية
                            if "لا توجد رسالتان كافيتان" not in message:
                                await self.client.send_message(
                                    chat_id,
                                    f"⚠️ خطأ في التحويل: {message}"
                                )

                    # الانتظار قبل التحقق مرة أخرى (مدة قصيرة للاستجابة السريعة)
                    await asyncio.sleep(2)

                except asyncio.CancelledError:
                    raise
                except Exception as e:
                    print(f"[م9] خطأ في التتبع: {e}")
            ...
            async with self.tracking_lock:
                key = (chat_id, target_user_id)
                if key in self.tracking_tasks:
                    # إزالة المهمة من القاموس إذا لم تكن قد أزيلت بالفعل
                    self.tracking_tasks.pop(key, None)
            ...
            try:
                ...
                await self.client.send_message(
                    chat_id,
                    f"🛑 توقف تتبع {display_name}"
                )
            except Exception:
                pass

            print(f"[م9] توقف تتبع المستخدم {target_user_id} في المحادثة {chat_id}")
    ...
    async def setup_handlers(self):
        """
        إعداد معالجات الأوامر الخاصة بالكلاس.
        """

        @self.client.on(events.NewMessage(pattern=r"^\.م9$"))
        async def help_menu(event):
            """
            عرض قائمة المساعدة عند كتابة الأمر .م9
            """
            if not self.running:
                return
...
◈ **`.تتبع`** ↳ بالرد على رسالة الشخص المراد تتبعه.

◈ **`.تتبع رابط_الرسالة`** ↳ لتتبع شخص عن طريق رابط إحدى رسائله.

◈ **`.سرعة 0`** ↳ إرسال الرسالتين مباشرة.

◈ **`.سرعة 1`** ↳ انتظار ثانية واحدة بين الرسالتين.

◈ **`.سرعة 2`** ↳ انتظار ثانيتين بين الرسالتين.

◈ **`.سرعة 3`** ↳ انتظار 8 ثوانٍ بين الرسالتين.

◈ **`.إيقاف_تتبع`** ↳ إيقاف تتبع الشخص المحدد (بالرد على رسالته أو باستخدام رابط رسالته).

◈ **`.ايقاف_تتبع`** ↳ نفس الأمر السابق.

━━━━━━━━━━━━
🔄 عند إرسال الشخص المتابع أي رسالة جديدة، يقوم البوت تلقائيًا بتحويل آخر رسالتين من الرسائل المحفوظة إلى نفس المجموعة.
"""
            await event.edit(help_text)

        @self.client.on(events.NewMessage(pattern=r"^\.تتبع(?:\s+(.*))?$"))
        async def start_tracking_cmd(event):
            """
            بدء التتبع: .تتبع (بالرد أو برابط)
            """
            if not self.running:
                return
            if event.sender_id != self.user_id:
            ...
            # محاولة استخراج المستخدم من الرد أو من الرابط
            user_id, username = await self._get_user_id_from_reply_or_link(event)

            if not user_id:
                await event.edit("⚠️ يرجى الرد على رسالة الشخص أو إرسال رابط إحدى رسائله.")
                return

            # التحقق من أن المستخدم ليس البوت نفسه
            if user_id == self.client.get_me().id:
                await event.edit("⚠️ لا يمكن تتبع نفسك!")
                return

            # بدء التتبع
            success, message = await self.start_tracking(chat_id, user_id, username)

            if success:
                await event.edit(f"✅ تم بدء تتبع {f'@{username}' if username else f'المستخدم {user_id}'} بنجاح!")
            else:
                await event.edit(f"⚠️ {message}")

        @self.client.on(events.NewMessage(pattern=r"^\.سرعة\s+(\d+)$"))
        async def set_speed_cmd(event):
            """
            تغيير سرعة التحويل: .سرعة 0-3
            """
            if not self.running:
                return
            if event.sender_id != self.user_id:
            ...
            if speed not in [0, 1, 2, 3]:
                await event.edit("⚠️ السرعة غير صحيحة. القيم المسموحة: 0, 1, 2, 3")
                return

            # تحويل السرعة إلى ثواني
            speed_mapping = {0: 0, 1: 1, 2: 2, 3: 8}
            actual_speed = speed_mapping[speed]

            chat_id = event.chat_id

            # محاولة استخراج المستخدم من الرد أو من الرابط
            user_id, username = await self._get_user_id_from_event(event)

            if not user_id:
                await event.edit("⚠️ يرجى الرد على رسالة الشخص أو كتابة @username لتحديد المستخدم.")
                return

            # تحديث السرعة
            success, message = await self.set_speed(chat_id, user_id, actual_speed)

            if success:
                await event.edit(f"✅ تم تحديث السرعة إلى {speed}")
            else:
                await event.edit(f"⚠️ {message}")

        @self.client.on(events.NewMessage(pattern=r"^\.(إيقاف_تتبع|ايقاف_تتبع)(?:\s+(.*))?$"))
        async def stop_tracking_cmd(event):
            """
            إيقاف التتبع: .إيقاف_تتبع (بالرد أو برابط)
            """
            if not self.running:
                return
            if event.sender_id != self.user_id:
            ...
            # إيقاف التتبع
            success, message = await self.stop_tracking(chat_id, user_id)

            if success:
                await event.edit(f"✅ تم إيقاف تتبع {f'@{username}' if username else f'المستخدم {user_id}'} بنجاح!")
            else:
                await event.edit(f"⚠️ {message}")

        # إضافة معالج للرسائل الجديدة من المستخدمين المتتبعين
        @self.client.on(events.NewMessage)
        async def handle_new_messages(event):
            """
            معالجة الرسائل الجديدة من المستخدمين المتتبعين.
            """
            if not self.running:
                return
            if event.out:
            ...
            # التحقق من وجود تتبع نشط لهذا المستخدم في هذه المحادثة
            async with self.tracking_lock:
                key = (chat_id, sender_id)
                if key not in self.tracking_tasks:
                    return
                # تحديث السرعة إذا تغيرت
                speed = self.tracking_tasks[key]["speed"]

            # إرسال إشارة بأن المستخدم أرسل رسالة جديدة (سيتم معالجتها في دالة التتبع)
            # ولكن يمكننا إضافة معالجة فورية هنا إذا أردنا
            # سنقوم بتحويل الرسائل فوراً كاستجابة سريعة

            try:
                # تحويل آخر رسالتين من المحفوظات
                success, message = await self._forward_last_two_saved_messages(
                    chat_id, sender_id, speed
                )
                ...
                print(f"[م9] خطأ في معالجة الرسالة الجديدة: {e}")

    async def stop(self):
        """
        إيقاف جميع عمليات التتبع عند إعادة التشغيل.
        """
        self.running = False
        await self.stop_all_tracking()

        print(f"[م9] تم إيقاف جميع عمليات التتبع للمستخدم {self.user_id}")
...
# ====================================================================
# تنظيف عمليات م9 عند إعادة التشغيل
# ====================================================================
async def stop_all_m9_tracking():
    """إيقاف جميع عمليات التتبع م9 عند إعادة التشغيل"""
    for client in all_clients:
        if hasattr(client, 'tracking_system'):
            await client.tracking_system.stop()

# تسجيل حدث لإيقاف التتبع عند الخروج
import atexit
def cleanup_m9():
    """دالة التنظيف عند خروج البرنامج"""
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            asyncio.create_task(stop_all_m9_tracking())
        else:
            loop.run_until_complete(stop_all_m9_tracking())
    except:
        pass

atexit.register(cleanup_m9)
